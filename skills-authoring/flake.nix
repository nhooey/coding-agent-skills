{
  description = "coding-agent-skills authoring skills: third-party Claude Code skills installed into the coding-agent-skills dev shell for authoring this repo — deliberately kept separate from the skills this repo outputs.";
  ...
  outputs =
    ...
    let
      ...
      agg = flake-skills.lib.mkAggregateSkillsFlake {
        ...
        # scoped to *these* authoring skills. The parent flake installs the
        # skills-git skills into the same project-scope dir under the default
        # `agent-skills-all` appName; a different name here keeps each
        # reconcile owning only its own slice (an entry the lock attributes
        # to another appName is left alone).
        name = "coding-agent-skills-authoring";
